...
== 信号结构
 ...
 当涉及用户态和内核态之间的信号信息传递时，RocketOS采用LinuxSigInfo结构作为标准化接口。LinuxSigInfo严格遵循POSIX标准规范，包含标准要求的所有字段和数据布局，确保与现有Linux应用程序的完全兼容。这种设计不仅体现在数据结构字段对应上，还包括内存布局、字节对齐以及各种信号类型特定信息的组织方式
...
== 信号发送
 ...
 对于线程级信号，也就是只针对特定线程的信号，系统会直接操作目标任务的信号待处理队列。通过op_sig_pending_mut方法获取任务的信号待处理状态的可变引用，然后调用add_signal方法将新的信号信息添加到待处理队列中。这个过程确保了信号不会丢失，即使当前任务正在执行其他操作。添加信号后，系统会检查当前任务是否满足可中断条件，这个检查通过check_interrupt方法完成，该方法会考虑任务当前的状态、信号屏蔽字以及其他相关因素来决定是否应该立即处理信号。

 对于进程级信号，处理过程更加复杂，因为这类信号需要影响整个进程组中的所有线程。系统会通过op_thread_group_mut方法获取进程组的可变引用，然后遍历进程组中的每一个任务。对于每个任务，系统都会执行与线程级信号相同的操作序列：将信号添加到任务的待处理队列中，检查中断条件，如果满足条件则设置中断标志、从等待队列中移除、设置为就绪状态并加入调度队列。这种设计确保了进程级信号能够被进程组中的任何一个线程处理，提高了信号处理的灵活性和效率。

 如果任务满足中断条件，系统会执行一系列操作来确保信号能够被及时处理。首先，系统会在信号待处理状态中设置中断标志，通过set_interrupted方法标记任务已被信号中断。然后调用delete_wait函数将任务从等待队列中移除，这是因为如果任务正在等待某个事件（比如I/O操作完成或者锁的释放），信号的到达应该能够中断这种等待状态。接着，系统会调用set_ready方法将任务状态设置为就绪，表示任务可以被调度器选中执行。最后，通过add_task方法将任务重新加入到调度器的就绪队列中，确保任务能够尽快获得CPU时间来处理信号。
...
=== 对SA_RESTART的特别处理
    SA_RESTART是Linux信号处理机制中一个重要的标志位，它主要用于解决信号与系统调用之间的交互问题，确保系统调用的连续性和可靠性。

    在linux中，SA_RESTART标志位实现是通过先进行重置再进行撤销而实现的，尽管能这样的做法能够保障信号处理的正确性，但相对的需要做一些浪费的保存操作，但在RocketOS中，我们通过对条件的判断来对是否进行重置操作的判断，判断的条件共需四条
   #pad(left: 4em)[
  + 需要重启的系统调用是否可以重启
...
=== sigframe结构设计
  ...
  #code-figure(
    ...
    当用户信号处理程序注册时设置了SA_SIGINFO标志位，情况变得更加复杂和功能丰富。在这种情况下，信号处理函数不仅需要接收信号编号，还需要获得包含详细信号信息的siginfo结构体以及完整的用户上下文信息。为了满足这些额外的需求，内核必须构造更为完整和复杂的SigRTFrame结构体。SigRTFrame结构体代表了信号处理机制的高级形态，它不仅包含了用于标识结构类型的flag字段，还整合了UContext结构体来提供完整的用户上下文信息，以及LinuxSigInfo结构体来传递详细的信号相关信息。这种设计使得信号处理函数能够获得更丰富的上下文信息，从而实现更精细化的信号处理逻辑。
  ...
  #figure(
    ...
    在用户信号处理流程结束后，会自动通过预先设置的跳板平台来调用sigreturn返回内核态，在sigreturn中通过在信号处理完成后检查sigframe中的flag字段，内核能够准确识别当前使用的结构体类型，从而选择相应的恢复策略来提取正确的上下文信息。这种设计不仅保证了不同信号处理模式下的功能正确性，还优化了系统资源的使用效率，避免了在简单信号处理场景中构造过于复杂的上下文结构。
