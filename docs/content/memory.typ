...
== 内存布局
#h(2em)在 RISC-V 架构下，内核例程与用户例程均采用 Sv39 分页机制进行地址映射，内核与用户共享同一个页表结构（即用户映射与内核映射共存于同一页表, 通过PTEFlags的U位进行隔离）。
...
#h(2em)在Loongarch架构下, 内核例程与用户例程处于不同的映射模式下：
...
== 堆分配器和用户页帧分配器
#h(2em)RocketOS 内核设计中，存在两个内存分配器，分别为 堆分配器 和 页帧分配器，其职责范围和实现机制有所不同：
- 堆分配器 负责内核动态内存的分配，管理位于内核镜像区域的内存空间，主要用于 kmalloc、对象创建等需要动态大小内存的场景。堆分配器对#link("https://crates.io/crates/buddy_system_allocator")[buddy_system_allocator]进行封装， 基于Buddy System 算法实现，支持不同大小块的合并与拆分，具备较好的内存利用率和碎片控制能力（如 #[ @堆分配器结构 ] 所示）。
...
#h(2em)一个虚拟地址空间由一个`MemorySet`对象描述（如#[@MemorySet对象]所示）。`MemorySet`对象包含了唯一的地址空间标识、区域映射表和页表帧数组等。区域集合`areas`是RocketOS 虚拟内存系统的核心数据结构，描述了虚拟地址空间的区域。不同的区域可能有不同的映射方式，例如线性映射、共享内存、写时复制等。页表帧数组用于存储页表帧的跟踪器。
...
#h(2em)在 RocketOS 中，虚拟地址空间的区域管理由 MapArea 结构体承担(如[@MapArea]所示)。它统一表示一段虚拟地址空间的区域，包括起始地址范围、权限、映射类型（如线性映射、按页分配、文件映射等），并提供了映射、取消映射、复制、扩展、分割等操作接口。

#h(2em)当前 RocketOS 支持的区域类型包括：
#pad(left: 4em)[
...
#h(2em)与整体地址空间解耦是 RocketOS 的一项重要设计。MapArea 并不持有页表根或页表帧等资源，而是通过传入的 PageTable 参数在映射和取消映射时完成操作。页帧的具体分配由上层的 MemorySet 管理。这种设计使得一个 MapArea 可被复制、移动并重用于不同的地址空间实例，有利于实现如 fork 中的地址空间复制、写时复制、懒分配高级功能。
...
#h(2em)主要用于 内核地址空间 的直接映射，比如将物理内存映射到虚拟地址（通常是KERNEL_BASE起始）。
...
#h(2em)用于用户进程的私有映射，如代码段、数据段、匿名映射等。
...
#h(2em)用户堆映射，支持扩展, 堆顶为`brk`，堆底为`heap_bottom`。与`Framed`共享很多行为，但更强调向上的扩展性

=== 文件映射区域

#h(2em)用户通过 mmap 创建的文件映射区域。
...
#h(2em)RocketOS 支持两种类型的共享内存：System V 共享内存和匿名共享内存。System V 共享内存由`shmat`系列系统调用产生，匿名共享内存由`mmap`系统调用产生。System V 共享内存通过`ShmSegment`对象管理，`SysVShm`对象包含了共享内存的 id 和对应分配的页帧，如#[@ShmSegment]所示。使用 Weak<Page> 而非 Arc<Page> 是为了将物理页的生命周期交由进程 attach/detach 控制，体现了“共享但非所有”的语义。
...
#h(2em)RocketOS 支持两种可恢复的缺页异常处理机制：写时复制（Copy-on-Write, COW） 和 懒分配（Lazy Allocation）。

=== 写时复制

#h(2em)写时复制机制用于进程创建后共享只读内存页（例如通过 `fork`），其页表项包含 `COW` 标志。当子进程试图写该页时： 若该页只被一个进程引用，直接清除 `COW` 标志，赋予写权限； 否则，分配一个新物理页，将原页内容复制到新页，并更新页表项。
...
#h(2em)懒分配机制适用于匿名区域（如堆、栈）和文件映射区域（如 `mmap`），延迟到首次访问再分配实际物理页。
