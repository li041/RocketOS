...
=== 任务切换设计
  ...
  在RocketOS任务切换过程中，内核栈的定位机制是通过Task结构体（任务控制块TCB）的首字段kstack实现的。该kstack字段作为记录指针，标记着对应任务内核栈的栈顶位置。
  ...
  #figure(
    image("img/任务切换示意图.png", width: 70%),
    caption: [任务切换示意图]
  ) <task_switch-figure>
...
=== 任务控制块设计
  ...
  在RocketOS的Task结构设计中，身份标识与层次关系通过三层标识体系得以实现，这一体系以tid、tgid和pgid为核心，确保了系统在支持现代多线程编程模型的同时，依然兼容传统的进程间关系。tid作为线程的唯一标识符，能够精确区分系统中每一个独立的线程执行单元；tgid则用于标识线程组，相当于传统意义上的进程概念，将属于同一进程的多个线程关联起来；而pgid则负责进程组的管理，用于协调一组相关进程的行为，例如信号传递或作业控制。此外，thread_group字段被设计用于管理同一进程内的所有线程，通过高效的线程组织方式，支持多线程任务的高效协同。
  ...
  在进程间继承关系方面，Task结构通过parent和children字段维护了一棵完整的进程树，其中parent指向父进程，children则记录子进程集合。为了避免循环引用问题，RocketOS采用了Rust语言的Arc（原子引用计数）和Weak智能指针，通过引用计数的动态管理，确保进程树结构的稳定性和内存安全。
...
=== 任务状态设计
  ...
  从Running状态出发存在多种转换路径。当任务主动调用yield让权或时间片耗尽时，会重新回到Ready状态等待下次调度。如果任务因为waitpid、文件I/O等操作需要等待资源时，会根据等待类型进入相应的阻塞状态：对于可以被信号中断的等待操作（如waitpid），任务进入Interruptable状态；对于关键的系统操作（如磁盘写入），任务进入UnInterruptable状态以确保操作的原子性。阻塞状态的唤醒机制体现了不同阻塞类型的特性差异。
...
=== TrapContext设计
  ...
  如@kernal_stack 所示，trap_context的结构设计，特别是tp寄存器的处理策略，主要是针对前述任务切换机制进行的专门优化。RocketOS在TrapContext结构体中引入了kernel_tp字段这一创新设计，虽然会产生轻微的存储开销，但实现了任务切换流程对传统堆内存分配和锁竞争机制的彻底规避。这一权衡设计的核心优势在于将任务上下文的访问模式从堆访问重构为栈访问，有效消除了动态内存分配、互斥锁获取与释放等高延迟操作，其性能收益远超额外存储开销所带来的成本，实现了整体系统效率的显著提升。
