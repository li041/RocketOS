...
=== TrapContext设计
  ...
  #h(2em)如@kernal_stack 所示，trap_context的结构设计，特别是tp寄存器的处理策略，主要是针对前述任务切换机制进行的专门优化。RocketOS在TrapContext结构体中引入了kernel_tp字段这一创新设计，虽然会产生轻微的存储开销，但实现了任务切换流程对传统堆内存分配和锁竞争机制的彻底规避。这一权衡设计的核心优势在于将任务上下文的访问模式从堆访问重构为栈访问，有效消除了动态内存分配、互斥锁获取与释放等高延迟操作，其性能收益远超额外存储开销所带来的成本，实现了整体系统效率的显著提升。
