...
#h(2em)RocketOS 的文件系统模块构建在一套抽象统一、模块解耦的 VFS 框架之上，支持挂载多个后端文件系统（如 Ext4、Tmpfs、Devfs 等），并通过统一的 Dentry、Inode、File 抽象，协调路径解析、权限控制与文件访问等操作流程。下图展示了 RocketOS 文件系统在内核中的总体架构，体现了从系统调用到具体后端文件系统的协作流程。
...
#h(2em)RocketOS 的挂载子系统由三类核心结构组成：`VfsMount` 表示一次具体的文件系统挂载，`Mount` 表示一个挂载点的元数据，`MountTree` 维护所有挂载关系的全局视图。三者协同构成一棵挂载树，实现文件系统命名空间的统一视图。

#h(2em)`VfsMount`封装了文件系统挂载的核心信息，包括挂载点的根目录、挂载的文件系统（超级块）以及挂载标志。`Mount`则表示一个具体的挂载点及其拓扑位置， `mountpoint`表示挂载操作发生的目录项，`vfs_mount`指向被挂载的文件系统实例，parent指向父挂载点，children存储子挂载点。`MountTree`则是一个全局的挂载点列表，维护所有`Mount`对象并提供必要的插入与查询接口。该结点通过全局静态变量`MOUNT_TREE`暴露，具备线程安全性。
...
#h(2em)系统启动时，通过 do_ext4_mount 函数挂载根文件系统。该函数执行以下步骤：
#pad(left: 4em)[
...
#h(2em) 在RocketOS 中，`inode` 抽象的核心体现在 `InodeOp` trait 接口之中。该接口定义了文件与目录的基本操作语义，包括读写、截断、页缓存交互、目录项查找与创建、符号链接、权限修改、文件状态查询等。每个具体的文件系统（如 Ext4）通过实现 `InodeOp` 接口，使其能以统一方式与 VFS 层交互，从而实现跨文件系统的透明访问能力。

#h(2em)为了支持运行时的类型动态调度与多文件系统扩展，`InodeOp` 被设计为 trait 对象（`dyn InodeOp`），并通过 `Arc<dyn InodeOp>` 进行引用计数与共享，确保线程安全与生命周期管理。
...
#h(2em) 以Ext4为例，Ext4Inode 实现了 InodeOp trait，并结合地址空间与块设备访问能力，具有如下核心特性
#pad(left: 2em)[
- 页缓存集成与统一地址空间管理：
...
== Dentry与目录树
#h(2em)Dentry（目录项）是文件系统中用于表示目录结构的核心数据结构。它将文件名与对应的 Inode 关联起来，形成一个树形结构。Dentry 的主要作用是加速文件路径解析和目录项查找。
...
#h(2em)为提高路径查找的性能，RocketOS 在 VFS 层引入 DentryCache。该缓存以路径字符串为键，存储解析过的 Dentry 对象，并通过引用计数与负目录项机制（用于标识不存在的路径），标识路径查找失败的目录项，用于避免重复错误查找，提升路径解析性能。在绝大多数文件访问场景中，路径查找都可在 DENTRY_CACHE 命中，大幅减少 inode 查找开销。

#h(2em) 同时为了确保VFS操作的安全性与合规性，Dentry提供了一套完整的权限检查逻辑：
...
#h(2em)在 RocketOS 的虚拟文件系统（VFS）框架中，File 结构体承担着进程级别的打开文件表示，是系统中从用户态调用（如 read, write, lseek, ioctl 等）到内核态 inode 操作之间的关键桥接抽象。其设计参考了 Linux 中 struct file 的语义，在类型安全与并发访问控制方面结合了 Rust 的所有权模型与同步原语。每一个打开的文件描述符在内核中都映射到一个唯一的 File 对象，封装了文件状态、访问语义与当前偏移量。

#h(2em)File 结构的核心包括一个受互斥锁保护的 FileInner 成员，后者保存了当前访问的 inode 引用、打开路径对应的 Path 结构体、打开标志（OpenFlags），以及动态维护的读写偏移量（offset）。该偏移量以字节为单位，反映了当前进程视角下对该文件的访问位置。在具有 O_APPEND 标志的场景下，偏移量初始化时即被设置为文件大小，以模拟追加写入的语义行为；否则偏移量初始化为零。
...
#h(2em)File 抽象在路径语义上也具备重要角色。每个 File 持有其打开路径对应的 Path 引用，而 Path 内部又可追溯至打开文件的 Dentry。通过此设计，File 可在运行时访问其路径相关元信息，如判断是否为目录（通过 is_dir 查询 Dentry 类型标志），执行目录遍历（如 readdir）等。此外，系统也提供了权限检查函数（如 readable, writable），用于根据打开标志与 O_PATH 模式等判断用户对文件的操作权限。

#h(2em)偏移量管理亦为 File 抽象中一项关键机制。除基本的 get_offset 与 add_offset 操作外，系统支持标准化的文件指针移动接口 seek，并扩展支持 `SEEK_DATA` 与 `SEEK_HOLE` 等现代语义，便于处理稀疏文件。在这类语义下，RocketOS 通过 inode 提供的页级映射信息（如 lookup_extent）判断实际存在的数据区域或空洞位置，从而实现对非连续数据布局的有效识别。

#h(2em)综合来看，File 抽象在 RocketOS 中不仅仅是打开文件的简单句柄，更是一个承载访问语义、权限标志、偏移控制与路径绑定的中间层，其设计显著提高了文件系统的通用性与模块化程度。通过与 inode、dentry、path 等子系统的紧密协作，File 为内核提供了一个精确、可扩展且高度类型安全的文件访问模型，适配多种文件类型与访问模式，为构建现代类 UNIX 操作系统内核的文件语义奠定了坚实基础。
...
#h(2em)整个文件系统框架强调模块解耦、抽象统一与类型安全，充分利用 Rust 所提供的is内存管理特性，在确保安全性的同时提升了系统可维护性与扩展性。通过这一体系，RocketOS 构建起一套具有现代操作系统特征的文件系统模型，为支持设备文件、网络文件系统、文件权限模型与高性能 I/O 提供了坚实基础。
