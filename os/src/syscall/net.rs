 ...
 * @LastEditTime: 2025-05-03 22:53:10
 * @FilePath: /RocketOS/os/src/syscall/net.rs
...
use alloc::vec;
use bitflags::Flags;
use crate::{arch::mm::{copy_from_user, copy_to_user},fs::fdtable::FdFlags, net::socket::{socket_address_from, socket_address_to, Domain, IpOption, Ipv6Option, Socket, SocketOption, SocketOptionLevel, SocketType, TcpSocketOption, SOCK_NONBLOCK}, task::{current_task, yield_current_task}};
...
pub fn syscall_socket(domain:usize,sockettype:usize,_protocol:usize)->SyscallRet{
    log::error!("[syscall_socket]:domain:{} sockettype:{}",domain,sockettype);
...
pub fn syscall_recv(socketfd:usize,buf:*mut u8,len:usize,_socketaddr:usize,_socketlen:usize,_flag:usize)->SyscallRet {
    ...
    log::error!("[syscall_recv]:sockfd:{:?},len:{:?}",socketfd,len);
    let task=current_task();
    let file=match task.fd_table().get_file(socketfd) {
        Some(f) => f,
        None => return Err(Errno::EBADF),
    ...
    Ok((size,_addr)) => {
        ...
        log::error!("[syscall_recv]:udp recv buf len {}",size);
        return Ok(size);
    },
    Err(e) => {
        Err(e)
...
pub fn syscall_shutdown(socketfd:usize)->SyscallRet {
    let task=current_task();
    let file=match task.fd_table().get_file(socketfd) {
        Some(f) => f,
        None => return Err(Errno::EBADF),
