...
use crate::{arch::mm::{copy_from_user, copy_to_user}, fs::{fdtable::FdFlags, file::{FileOp, OpenFlags}, pipe::{self, make_pipe}, uapi::IoVec}, net::{addr::{from_ipendpoint_to_socketaddr, LOOP_BACK_IP}, socket::{check_alg, socket_address_from, socket_address_from_af_alg, socket_address_from_unix, socket_address_to, ALG_Option, Domain, IpOption, Ipv6Option, MessageHeaderRaw, Socket, SocketOption, SocketOptionLevel, SocketType, TcpSocketOption, SOCK_CLOEXEC, SOCK_NONBLOCK}}, syscall::task::sys_nanosleep, task::{current_task, yield_current_task}};
...
pub fn syscall_sendmsg(socketfd:usize,msg_ptr:usize,flag:usize)->SyscallRet {
    ...
    copy_from_user(msg_ptr as *const MessageHeaderRaw,
    ...
    assert!(iovec_count <= 1, "temperrily Only one iovec is supported in sendmsg syscall");
    ...
    if user_hdr.name_len>0 {
        copy_from_user(user_hdr.name as *const u8, kernel_name.as_mut_ptr(), user_hdr.name_len as usize)?;
    }
    if user_hdr.iovec_len>0 {
        ...
        log::error!("[syscall_sendmsg]:kernel_iovec base:{:?} kernel_iovec len:{:?}",kernel_iovec[0].base,kernel_iovec[0].len);
    }
    if user_hdr.control_len>0 {
        copy_from_user(user_hdr.control as *const u8, kernel_control.as_mut_ptr(), user_hdr.control_len as usize)?;
    }
    ...
    if kernel_iovec[0].len != 0 {
        copy_from_user(kernel_iovec[0].base as *const u8, kernel_buf.as_mut_ptr(), kernel_buf.len())?;
...
pub fn syscall_recvmsg(socketfd: usize, msg_ptr: usize, _flags: usize) -> SyscallRet {
    ...
    log::error!("[syscall_recvmsg]: socketfd: {}, msg_ptr: {}", socketfd, msg_ptr);
