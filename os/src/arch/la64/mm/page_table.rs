...
impl PageTable {
    ...
    // 目前仅用在映射sigreturn_trampoline时，权限可能存在问题
    pub fn update_flags(&mut self, vpn: VirtPageNum, flags: PTEFlags) {
        ...
        // 去除R、X、W
        full_flag.insert(PTEFlags::NR| PTEFlags::NX);
        full_flag.remove(PTEFlags::W);
        // 加上新的权限
        if !flags.contains(PTEFlags::NR) { full_flag.remove(PTEFlags::NR); }
        if !flags.contains(PTEFlags::NX) { full_flag.remove(PTEFlags::NX); }
