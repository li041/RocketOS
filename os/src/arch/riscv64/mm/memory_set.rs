...
//! MapType
//! MapPermision
use core::{arch::asm, mem, panic};

use super::{PageTable, PageTableEntry, PhysPageNum, StepByOne, VPNRange, VirtAddr, VirtPageNum};

use crate::{
    config::{MMAP_MIN_ADDR, PAGE_SIZE_BITS, USER_STACK_SIZE}, drivers::get_dev_tree_size, fs::namei::path_openat, task::{aux::*, current_task}, utils::ceil_to_page_size
};
use alloc::{
...
impl MemorySet {
    ...
    pub fn new_kernel() -> Self {
        ...
        // let mut vpn:VirtPageNum=(KERNEL_BASE+0xbfe00000).into();
        // let mut ppn:PhysPageNum=(0xbfe00000).into();
        // ///映射dev_tree
        // for i in 0..10{
        //     memory_set.page_table.map(vpn, ppn, PTEFlags::R|PTEFlags::W);
        //     vpn.step();
        //     ppn.step();
        // }
        let dev_tree_size=get_dev_tree_size(0xbfe00000 as usize);
        memory_set.push_with_offset(MapArea::new_from_va((KERNEL_BASE+0xbfe00000).into(), (KERNEL_BASE+0xbfe00000+dev_tree_size).into(), MapType::Linear, MapPermission::R|MapPermission::W), None, 0);
...
impl MemorySet {
    ...
    pub fn push_with_offset(&mut self, mut map_area: MapArea, data: Option<&[u8]>, map_offset: usize) {
