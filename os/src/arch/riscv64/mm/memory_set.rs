...
//! MapType
//! MapPermision
use core::{arch::asm, mem, panic, ptr::copy_nonoverlapping};

use super::{PageTable, PageTableEntry, PhysPageNum, StepByOne, VPNRange, VirtAddr, VirtPageNum};

use crate::{
    arch::{config::{MMAP_MIN_ADDR, PAGE_SIZE_BITS, USER_STACK_SIZE}, trampoline::sigreturn_trampoline}, fs::namei::path_openat, task::{aux::*, current_task}, utils::ceil_to_page_size
};
use alloc::{
...
impl MemorySet {
    ...
    pub fn from_elf(elf_data: &[u8]) -> (Self, usize, usize, usize, Vec<AuxHeader>) {
        ...
        if need_dl {
            ...
            // 加载动态链接器
            if let Ok(interpreter) = path_openat(&interpreter, 0, AT_FDCWD, 0) {
                ...
                interp_entry = interp_head.pt2.entry_point() as usize + DL_INTERP_OFFSET;
        ...
        if need_dl {
            log::error!("[from_elf] dynamic-linker entry_point: {:#x}", interp_entry);
            return (memory_set, satp, ustack_top, interp_entry, aux_vec);
        } else {
