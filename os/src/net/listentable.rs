 ...
 * @LastEditTime: 2025-05-03 21:30:43
 * @FilePath: /RocketOS/os/src/net/listentable.rs
    ...
    pub fn accept(&self, port: u16) -> Result<(SocketHandle, (IpEndpoint, IpEndpoint)),Errno> {
        if let Some(entry) = self.table[port as usize].lock().deref_mut() {
            ...
            let idx = syn_queue
                ...
                .ok_or(Errno::EAGAIN).unwrap(); // wait for connection
            // if let Ok(idx)=idx {
            //     log::warn!(
            ...
            if is_closed(handle) {
                panic!("socket accept() failed: connection reset")
            } else {
    ...
    pub fn push_incoming_packet(&self,dst:IpEndpoint,src:IpEndpoint,sockets:&mut SocketSet) {
        ...
        if let Some(entry) = self.table[dst.port as usize].lock().deref_mut() {
            ...
            if socket.listen(entry.listen_endpoint).is_ok() {
                println!("[push_incoming_packet]:socket listen_endpoint {:?}",entry.listen_endpoint);
                let handle = sockets.add(socket);
                println!("[push_incoming_packet]:socket handle {:?}",handle);
