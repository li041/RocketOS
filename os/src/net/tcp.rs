 ...
 * @LastEditTime: 2025-05-04 23:50:39
 * @FilePath: /RocketOS/os/src/net/tcp.rs
...
use core::{cell::UnsafeCell, net::SocketAddr,sync::atomic::{AtomicBool, AtomicU8, Ordering}};
use smoltcp::{iface::SocketHandle, socket::tcp::{self, ConnectError, RecvError, SendError}, wire::{IpEndpoint, IpListenEndpoint, Ipv4Address}};
use spin::Mutex;

use crate::{net::{addr::{from_sockaddr_to_ipendpoint, is_unspecified}, ETH0, LOOPBACK}, syscall::errno::Errno, task::{current_task, yield_current_task}};
...
impl TcpSocket {
    ...
    pub fn connect(&self,remote_addr:SocketAddr)->Result<(),Errno> {
        ...
        if self.is_nonblocking() {
            ...
            self.block_on(||{
                ...
                println!("[Tcpsocket_connect]:still in block on connect");
    ...
    pub fn with_socket_mut<F,T>(&self,f:F)->T
    where F:FnOnce(&mut tcp::Socket)->T
    {
        let handle=unsafe { self.handle.get().read().unwrap() };
        SOCKET_SET.with_socket_mut::<_,tcp::Socket,_>(handle, |socket|{
            f(socket)
