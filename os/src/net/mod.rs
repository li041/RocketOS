 ...
 * @LastEditTime: 2025-05-03 21:31:52
 * @FilePath: /RocketOS/os/src/net/mod.rs
...
use smoltcp::{iface::{Config, Interface, SocketHandle, SocketSet}, phy::{Device, DeviceCapabilities, Medium, RxToken, TxToken}, socket::{ tcp::SocketBuffer, AnySocket, Socket}, storage::{PacketBuffer, PacketMetadata}, wire::{EthernetAddress, EthernetFrame, HardwareAddress, IpCidr, IpProtocol, Ipv4Packet, TcpPacket}};
// use socket::Socket;
use spin::Mutex;
use virtio_drivers::transport::{mmio::MmioTransport, pci::PciTransport, Transport};
use crate::{arch::timer::get_time, drivers::net::{netdevice::{NetBufPtr, NetDevice}, VirtioNetDevice}, syscall::errno::Errno, task::yield_current_task};
...
pub fn init(net_device:Option<VirtioNetDevice<32,HalImpl,MmioTransport>>) {
    ...
    if let Some(dev) = net_device {
        ...
        let eth0=InterfaceWrapper::new("eth0", ether_addr, NetDeviceWrapper { inner: RefCell::new(Box::new(dev)) });
...
impl<'a> SocketSetWrapper<'a> {
    ...
    pub fn new_udp_socket()->smoltcp::socket::udp::Socket<'a> {
        ...
        // 第二个参数指定了每个数据包最大数据量
        let udp_recv_buffer=PacketBuffer::new(vec![PacketMetadata::EMPTY;256],vec![0;UDP_RX_BUF_LEN]);
        let udp_send_buffer=PacketBuffer::new(vec![PacketMetadata::EMPTY;256],vec![0;UDP_TX_BUF_LEN]);
    ...
    pub fn poll_interfaces(&self) {
        ...
        let b=LOOPBACK.lock().poll(SmolInstant::from_micros_const((get_time()/1000) as i64),
        LOOPBACK_DEV.lock().deref_mut(), 
    &mut self.0.lock());
    ...
    pub fn bind_check(&self, addr: IpAddress, port: u16) -> Result<(), Errno> {
...
fn snoop_tcp_packet(buf: &[u8], sockets: &mut SocketSet<'_>) -> Result<(), smoltcp::wire::Error> {
    ...
    if ipv4_packet.next_header() == IpProtocol::Tcp {
        ...
        log::error!("[snoop_tcp_packet]:tcp_packet syn is {:?},ack is {:?}",tcp_packet.syn(),tcp_packet.ack());
        if is_first {
...
impl<'a> InterfaceWrapper {
    fn new(name:&'static str,address:EthernetAddress,mut dev:NetDeviceWrapper)->Self {
        ...
        let iface=Mutex::new(Interface::new(config, &mut dev, SmolInstant::from_micros_const(get_time() as i64)));
        InterfaceWrapper { iface: iface, address: address, name: name, dev:Mutex::new(dev) }
    }
    fn name(&self)->&str{
        self.name
    }
    ...
    pub fn set_gatway(&self,gateway_ipv4:IpAddress,gateway_ipv6:IpAddress) {
        ...
        if let IpAddress::Ipv4(v4)=gateway_ipv4 {
            self.iface.lock().routes_mut().add_default_ipv4_route(v4).unwrap();
        }
        if let IpAddress::Ipv6(v6)=gateway_ipv6 {
            self.iface.lock().routes_mut().add_default_ipv6_route(v6).unwrap();
        }
    }
