 ...
 * @LastEditTime: 2025-05-03 23:53:48
 * @FilePath: /RocketOS/os/src/net/socket.rs
 * @Description: socket file
 * 
 * Copyright (c) 2025 by peterluck2021@163.com, All Rights Reserved. 
 */

use core::{net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr, SocketAddrV4, SocketAddrV6}, ptr::copy_nonoverlapping, sync::atomic::{AtomicBool, AtomicU64}};

use alloc::{string::String, vec::Vec};
use alloc::vec;
use num_enum::TryFromPrimitive;
use smoltcp::{socket::tcp, wire::{IpAddress, Ipv4Address}};
use spin::Mutex;
use crate::{arch::{config::SysResult, mm::copy_to_user}, fs::file::OpenFlags, net::udp::get_ephemeral_port, timer::TimeSpec};

use crate::{arch::{mm::copy_from_user}, fs::file::FileOp, mm::VirtPageNum, syscall::errno::{Errno, SyscallRet}};

use super::{add_membership, addr::from_ipendpoint_to_socketaddr, poll_interfaces, tcp::TcpSocket, udp::UdpSocket};
...
impl Socket {
    ...
    pub fn shutdown(&self)->Result<usize,Errno> {
        match &self.inner {
            SocketInner::Tcp(tcp_socket) => tcp_socket.close(),
            SocketInner::Udp(udp_socket) => udp_socket.shutdown(),
        }
        Ok(0)
    }
...
pub fn socket_address_to(sockaddr: SocketAddr, addr: usize, addr_len: usize)->SyscallRet {
    ...
    let data: Vec<u16> = match sockaddr {
        SocketAddr::V4(v4) => {
            ...
            let domain = (Domain::AF_INET as u16).to_be();
            // 端口号转换为大端序
    ...
    pub fn set(&self,socket: &Socket,opt:&[u8])->SyscallRet{
        match self {
            SocketOption::SO_REUSEADDR => {
                ...
                println!("[set_reuse_addr] reuse addr is {:?}",addr);
            ...
            SocketOption::SO_KEEPALIVE => {
                ...
                match &socket.inner {
                    SocketInner::Tcp(tcp_socket) => {
                        tcp_socket.with_socket_mut(|s|{
                            s.set_keep_alive(interval);
                        })
                    },
                    SocketInner::Udp(udp_socket) => {
