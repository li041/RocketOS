...
use crate::{config::KERNEL_BASE, mm::{MapArea, MapPermission, MapType, VPNRange, VirtAddr, KERNEL_SPACE}};
 ...
 * @Date: 2025-03-24 20:52:42
 * @LastEditors: Peter/peterluck2021@163.com
 * @LastEditTime: 2025-03-26 10:00:58
 * @FilePath: /RocketOS/os/src/drivers/net/mod.rs
 * @Description: 
 * 
...
pub fn init_net_device(dtb_addr:usize) {
    ...
    //表示设备地址和长度占用32字长个数
    // println!("fjisof");
    let address_cells=dev_tree.root().properties().find(|prop|prop.name=="#address-cells").unwrap().value[3];
    let size_cells=dev_tree.root().properties().find(|prop|prop.name=="#size-cells").unwrap().value[3];
        ...
        if node.name=="soc" {
            for node_t in node.children() {
                if node_t.name=="virtio_mmio@10008000" {
                    ...
                    KERNEL_SPACE.lock().push_with_offset(MapArea::new(
                        VPNRange::new(
                    ...
                    NET_DEVICE_ADDR.lock().replace((KERNEL_BASE+mmio_base).into());
...
pub fn init_net_dev_la<T: Transport+'static>(transport: T){
    log::error!("[init_net_dev_la]:the transport device_type is {:?}",transport.device_type());
...
fn parse_reg(node: &FdtNode, addr_cells: usize, size_cells: usize) -> Vec<(usize, usize)> {
    let reg = node.properties().find(|prop|prop.name=="reg").unwrap().value;
...
/// 这里使用Hal,Transport Const QS是因为inner VirtioNetRaw需要使用
...
impl<const QS:usize,H:Hal,T:Transport> VirtioNetDevice<QS,H,T> {
    ...
    pub fn new(transport:T)->Self {
        ...
        let inner=VirtIONetRaw::<H,T,QS>::new(transport).map_err(|e| {
            log::error!("Failed to create VirtIONetRaw: {:?}", e)}).unwrap();
        ...
        for (i,buf) in net_dev.recv_buffers.iter_mut().enumerate(){
            ...
            let token=unsafe { net_dev.inner.receive_begin(alloc_buf.get_raw_mut_buf()).unwrap()};
        ...
        for _ in 0..QS{
            ...
            let header_len=net_dev.inner.fill_buffer_header(alloc_buf.get_raw_mut_buf()).unwrap();
...
impl<const QS:usize,H:Hal,T:Transport> NetDevice for VirtioNetDevice<QS,H,T> {
    ...
    fn recycle_recv_buffer(&mut self,recv_buf_ptr:NetBufPtr) {
        ...
        let token=unsafe { self.inner.receive_begin(recv_buf.get_raw_mut_buf()).unwrap() };
        ...
        log::error!("[recycle_recv_buffer]:after recyble recv buf is {:?}",recv_buf.get_packet());
    ...
    fn recycle_send_buffer(&mut self)->Result<(),()>{
        while let Some(token)=self.inner.poll_transmit(){
            ...
            if let Some(send_buf)=self.send_buffers[token as usize].take(){
                unsafe { let _ = self.inner.transmit_complete(token, send_buf.get_packet_with_header()); };
    ...
    fn send(&mut self,ptr:NetBufPtr) {
        ...
        let token=unsafe { self.inner.transmit_begin(send_netbuf.get_packet_with_header()).unwrap() };
        log::error!("[VirtioNetDev_send]:send buf {:?}",send_netbuf.get_packet());
    ...
    fn recv(&mut self)->Option<NetBufPtr> {
        ...
        if let Some(token) =  self.inner.poll_receive(){
            ...
            let (header_len,packet_len)=unsafe { self.inner.receive_complete(token, recv_buf.get_raw_mut_buf()).unwrap() };
            recv_buf.set_header_len(header_len);
            recv_buf.set_packet_len(packet_len);
            log::error!("[VirtioNetDevice_recv]:recv_buf is {:?}",recv_buf.get_packet());
...
impl NetBuf {
    ...
    ///这个函数主要在receive_complete时获得header_len和package_len使用，所以必须判断不能能够超过buf_len
...
impl NetBufPool {
    ...
    pub fn new(capacity:usize,buf_len:usize)->Arc<Self> {
        ...
        Arc::new(
            NetBufPool { capacity: capacity, buf_len: buf_len, pool: pool, free_list: Mutex::new(free_list) })
    }
    ...
    pub fn alloc(self:&Arc<Self>)->NetBuf {
        ...
        NetBuf { header_len: 0, packet_len: 0, capacity: self.buf_len, buf_ptr: buf_ptr,pool_offset:offset,pool:Arc::clone(self) }
    }
...
//     fn dma_alloc(pages: usize, direction: virtio_drivers::BufferDirection) -> (virtio_drivers::PhysAddr, NonNull<u8>) {
//         todo!()
//     }
